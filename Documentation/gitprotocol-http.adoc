...
Servers MAY respond with other HTTP status codes to indicate specific error
conditions such as `413 Payload Too Large` or `429 Too Many Requests` with
`Content-Type: application/x-$servicename-advertisement`. In this case the
response MUST contain an error. Clients MUST parse such a response the same as
if it came from a `200 OK`.
